...
interface DataTableProps<T> {
  ...
  /** Custom delete confirmation message. Default: "确定要删除该记录吗？" */
...
export default function DataTable<T extends Record<string, any>>({
  ...
  const getConfirmMessage = (record: T): string => {
    ...
    // Try to use the record's name field, fallback to default
    const name = record.name ?? "";
    return name ? `确定要删除"${name}"吗？此操作不可撤销。` : "确定要删除该记录吗？此操作不可撤销。";
  };
