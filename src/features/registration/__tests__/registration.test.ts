...
// Mock next/cache to prevent revalidatePath errors in vitest
vi.mock("next/cache", () => ({
...
describe("createRegistration", () => {
  ...
  it("should create a registration successfully", async () => {
    ...
    const result = await createRegistration(
      "patient-1",
      { scheduleId: "schedule-1", profileId: "profile-1", type: "normal" },
    );
  ...
  it("should throw ConflictError when schedule is full", async () => {
    ...
    await expect(
      createRegistration("patient-1", { scheduleId: "schedule-1", profileId: "profile-1", type: "normal" }),
...
describe("listRegistrations", () => {
  ...
  it("should return paginated registrations for a patient", async () => {
    ...
    const result = await listRegistrations({ patientId: "patient-1" });
  ...
  it("should filter by status when provided", async () => {
    ...
    const result = await listRegistrations({ patientId: "patient-1", status: "pending" });
  ...
  it("should apply pagination correctly", async () => {
    ...
    await listRegistrations({ patientId: "patient-1", page: 3, pageSize: 5 });
