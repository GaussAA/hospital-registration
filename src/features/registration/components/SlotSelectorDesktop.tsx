...
export default function SlotSelectorDesktop({
  ...
  return (
    <div className="hidden md:block bg-card rounded-2xl border border-border overflow-hidden shadow-sm card-hover">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gradient-to-r from-blue-50/80 to-indigo-50/80 dark:from-blue-950/20 dark:to-indigo-950/20">
              <th className="sticky left-0 z-10 bg-inherit px-3 py-3.5 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider min-w-[60px]">
                时段
              </th>
              {dateRange.map((day) => {
                ...
                return (
                  ...
                  >
                    <div className="text-sm text-foreground">{`${day.getMonth() + 1}/${day.getDate()}`}</div>
                    <div className="text-xs mt-0.5 text-muted-foreground">{weekDayNames[day.getDay()]}</div>
                  </th>
                );
              })}
            </tr>
          ...
          <tbody>
            {timeSlots.map((slot) => (
              <tr key={slot} className="border-t border-border/60 hover:bg-accent/50 transition-colors">
                <td className="sticky left-0 z-10 bg-card px-3 py-3 text-sm font-medium text-muted-foreground whitespace-nowrap">
