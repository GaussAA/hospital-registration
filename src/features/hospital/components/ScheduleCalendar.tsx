  ...
  return (
    <div className="bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
      <div className="p-4 border-b border-gray-100 dark:border-gray-700/50">
        ...
        <p className="text-sm text-[var(--muted-foreground)] mt-1">未来 7 天出诊信息</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-800">
              <th className="px-4 py-3 text-left text-[var(--muted-foreground)] font-medium">时段</th>
              {days.map((day) => (
                <th
                  key={day.toISOString()}
                  className={`px-4 py-3 text-center font-medium ${
                    isSameDay(day, today) ? "text-blue-600 dark:text-blue-400" : "text-[var(--muted-foreground)]"
                  }`}
                >
                        ...
                        <div className="flex flex-col gap-1.5">
                          {daySlots.map((s) => (
                            ...
                            >
                              ...
                              <div className={selectedSlotId === s.id ? "text-blue-100" : s.remaining > 0 ? "text-blue-500 dark:text-blue-400" : "text-[var(--muted-foreground)]"}>
          ...
          return (
            <div key={day.toISOString()} className="border border-gray-100 dark:border-gray-700/50 rounded-lg p-3">
              ...
              {dayAllSlots.length > 0 ? (
                ...
                <span className="text-xs text-[var(--muted-foreground)]">暂无排班</span>
              )}
            </div>
          );
        })}
