    ...
    <div className="bg-white dark:bg-[#1e293b] rounded-xl border border-gray-200 dark:border-gray-700 p-6">
      <div className="flex items-start gap-5 mb-6">
        ...
        <div className="shrink-0 w-20 h-20 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center overflow-hidden">
          {doctor.avatarUrl ? (
            ...
            <Image src={doctorAvatarFallback} alt={doctor.name} width={80} height={80} className="object-cover" />
          )}
        </div>
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">
            <span className="font-medium text-foreground">专长：</span>
            {doctor.specialty}
          </p>
        </div>
      </div>
      ...
      {doctor.introduction && (
        <div className="border-t border-gray-100 dark:border-gray-700/50 pt-4">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-line">{doctor.introduction}</p>
        </div>
      )}
    </div>
  );
