...
export async function handleCreateRegistration(
  ...
  try {
    ...
    return (
      `✅ 挂号成功！\n\n` +
      `🏥 医院：${(registration as RegistrationDisplay).doctor.department.hospital.name}\n` +
      `📋 科室：${(registration as RegistrationDisplay).doctor.department.name}\n` +
      `👨‍⚕️ 医生：${(registration as RegistrationDisplay).doctor.name}（${(registration as RegistrationDisplay).doctor.title}）\n` +
      `🧑 就诊人：${(registration as RegistrationDisplay).profile.name}\n` +
...
export async function handleListRegistrations(
  ...
  return (
    ...
    result.list
      .map(
        (r: RegistrationDisplay, i: number) =>
          `${i + 1}. 🆔 ${r.id}\n` +
